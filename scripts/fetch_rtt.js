...
// Requires env vars: RTT_USERNAME / RTT_PASSWORD
...
const GBTT = process.env.BOOKED_DEPART_HHMM || "0744"; // booked departure
...
// Rule: show today's 07:44 until 17:00 local, then tomorrow's
function chooseServiceDate() {
  const now = new Date();
  const lp = getLocalParts(now);
  const hour = parseInt(lp.hour, 10);
  let y = parseInt(lp.year, 10),
    m = parseInt(lp.month, 10),
    d = parseInt(lp.day, 10);
  if (hour >= 17) {
    const dt = new Date(Date.UTC(y, m - 1, d, 12, 0, 0));
    dt.setUTCDate(dt.getUTCDate() + 1);
    const nlp = getLocalParts(dt);
    y = parseInt(nlp.year, 10);
    m = parseInt(nlp.month, 10);
    d = parseInt(nlp.day, 10);
  }
...
const generatedAt = new Date(
  `${nowLP.year}-${nowLP.month}-${nowLP.day}T${nowLP.hour}:${nowLP.minute}:00`
).toISOString();
...
async function searchCandidates() {
  ...
  try {
    ...
    const svcs = (js?.services || []).filter(
      (s) => s?.locationDetail?.gbttBookedDeparture === GBTT
    );
...
(async () => {
  try {
    ...
    if (origin?.isCancelled || dest?.isCancelled) overall = "cancelled";
      ...
      (origin?.departureDelayMins ?? 0) > 0 ||
      (dest?.arrivalDelayMins ?? 0) > 0
    )
      overall = "delayed";
    else if (
      (origin?.departureDelayMins ?? 0) < 0 ||
